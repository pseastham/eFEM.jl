...
# exports surface in 3D, where connectivity matrix has been generated by mesh function
function SURF_UNSTRUCTURED_TO_VTK(xyz::Matrix{Float64},cm::Matrix{Int},
                                  fval,fname,vecVal,vecName,
                                  file_name::String;time=0.0)
  # prepare
  fvalPresent   = false
  ...
  println(f,"POINTS ",Npts," float")
  for i=1:Npts
    println(f,xyz[i,1]," ",xyz[i,2]," ",xyz[i,3])
  end
  ...
  if vecValPresent
    for j=1:NumVecVals
      println(f,"VECTORS ",vecName[j]," double")
      for i=1:Npts
        println(f,vecVal[i,1,j]," ",vecVal[i,2,j]," ",vecVal[i,3,j])
      end
    end
  end
...
function SURF_UNSTRUCTURED_TO_VTK(xyz::Matrix{Float64},cm::Matrix{Int},
                                  fval,fname,file_name::String;time=0.0)
  vName = []; vVal  = []
  return SURF_UNSTRUCTURED_TO_VTK(xyz,cm,fval,fname,vVal,vName,file_name;time=time)
end
function SURF_UNSTRUCTURED_TO_VTK(xyz::Matrix{Float64},cm::Matrix{Int},file_name::String;time=0.0)
  fName = []; fVal  = []
  vName = []; vVal  = []
  return SURF_UNSTRUCTURED_TO_VTK(xyz,cm,fVal,fName,vVal,vName,file_name;time=time)
end
